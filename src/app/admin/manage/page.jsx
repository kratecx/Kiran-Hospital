...
export default function UnifiedAdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState("admissionforms"); // "admissionforms" or "contactforms"
  const [dataList, setDataList] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [editingId, setEditingId] = useState(null);

  // Form State for Admission Forms
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const endpoint = activeTab === "admissionforms" ? "/api/admissionforms" : "/api/contactforms";
      const res = await fetch(`${endpoint}?secret=${secret}&id=${id}`, {
        method: "DELETE",
      });
      const result = await res.json();
      if (!res.ok) throw new Error(result.error);
